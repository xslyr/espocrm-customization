...
/**
 * Disabled AssignedUser fields for entity types with AssignedUsers enabled.
 * @noinspection PhpUnused
 */
class DisableAssignedUser implements AdditionalBuilder
{
    public function build(stdClass $data): void
    {
        ...
        foreach (get_object_vars($scopes) as $scope => $itemDefs) {
            ...
            $assignedUserFieldDefs = $fieldsDefs->assignedUser ?? null;

            if (!$assignedUserFieldDefs instanceof stdClass) {
                return;
            }

            $assignedUserFieldDefs->disabled = true;
        }
    }
}
